  ...
  new Promise((resolve, reject) => {
    ...
    downImage(url).then(imgPath =>
      Taro.getImageInfo({ src: imgPath })
        .then(imgInfo => {
          console.debug('获取图片信息: ', item, imgInfo);
          // 获取图片信息
          ...
          const sw = imgWidth * ratio;
          const sh = imgHeight * ratio;

          sx = x + Math.floor((width - sw) / 2);
          sy = y + Math.floor((height - sh) / 2);

          console.debug('图片缩放信息', sx, sy, sw, sh);

          // sx = Math.floor((width - imgWidth * ratio) / 2);
          // sy = Math.floor((height - imgHeight * ratio) / 2);
