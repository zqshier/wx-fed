<template>
  <view class="nut-flex-col nut-col-center mg-bottom-16" :class="$style.coupon">
    <view
      :class="[$style['coupon-warp'], { [$style['coupon-warp_disable']]: isDisable }]"
      class="box nut-flex-col nut-col-top nut-row-between"
    >
      <view :class="$style['coupon-top']" class="nut-flex nut-col-top nut-row-between">
        <view class="f_s-12 f_w-400">
          <view class="f_s-16 f_w-600" :class="$style['top_type']"
            >满减券 ｜ {{ isAvailableStatus ? '部分可用' : '全场通用' }}
          </view>

          <block v-if="itemData?.start">
            <view class="nut-flex f_s-12 f_w-400" :class="{ [$style.time_border]: isAvailableStatus }">
          ...
          <block v-else>
            ...
            >
            <view class="nut-flex f_s-11 f_w-400" :class="{ [$style.time_border]: isAvailableStatus }" v-else>{{
              expirationText
            }}</view>
          </block>
        ...
        <view class="nut-flex-col nut-col-bottom">
          <view class="f_s-20 f_w-500 numfnt-medium" :class="$style['top_denomination']"
            >¥{{ formatPrice(itemData.value) }}</view
          >
          <view class="f_s-12 f_w-400" :class="$style['top_conditions']"
      ...
      <view :class="$style['coupon-end']" class="nut-flex nut-col-bottom nut-row-between">
        <view :class="$style['end_usageRules']" class="f_s-12 f_w-400 nut-flex nut-row-center" @tap="handleRule">
    ...
    <view class="f_s-10 f_w-400" :class="[$style['rule_state'], isUnfold ? $style['info_rule-none'] : '']"
...
const handleClick = async () => {
  ...
  linkflow &&
    linkflow.track({
      event: ELinkflowEventsName.CouponCenter_Coupon_Click,
      props: { buttonName: statusText.value, triggerPagePath } as ILinkflowEventsProps,
    });
  ...
  checkMemberLevel(LOGIN_TARGET.coupon, handleClick);
...
const handleGet = async (templateId?: string) => {
  ...
  try {
    const result = await useCouponStore().pull({
    ...
    emits('userStateChanged', 2, props.index, result);
  } catch (error) {
